<?php
    ...
    */
    if(isset($_POST['assign'])) {
      ...
      $user = $_POST['it_users'];
      $ticketID = $_POST['id'];
      $sql = "UPDATE `tickets` SET `assigned_to` = '$user'
        WHERE `tickets`.`ticket_id` = $ticketID";
      
      if(mysqli_query($con, $sql)) {
        // Ticket was successfully updated
        echo '<script>alert("Ticket successfully reassigned")</script>';
...
<head>
  ...
  <title>IT Support Manager "View All Pending Tickets" Page</title>    
...
<div class="main">
<h3>This is the testing "view all pending tickets" page for the IT Support Manager users.</h3>
<br>

<?php

    // Find all the tickets with a status of "Pending" in the DB
    $query = "SELECT ticket_id, date_created, priority, created_by, description, status, assigned_to 
